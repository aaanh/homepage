import { z } from "zod";
import { Experience, ExperienceSchema } from "./commons";

export default function getExperiences() {
  const experiences: Experience[] = [
    {
      organization: "Ericsson",
      link: "https://ericsson.com",
      role: "Software Developer",
      type: "",
      location: "Canada",
      startMonth: 5,
      startYear: 2025,
      endYear: 0,
      description: [
        "Internal and open-source tooling and platform development",
      ],
      skills: [
        "Java",
        "TypeScript",
        "React",
        "Database",
        "Tracing",
        "Full-stack",
        "Security",
      ],
    },
    {
      organization: "Brake",
      link: "https://wbrake.com",
      role: "Software Developer",
      type: "Permanent, Part-time",
      location: "Canada",
      startMonth: 4,
      startYear: 2024,
      endYear: 0,
      description: ["Web application development"],
      skills: ["Typescript", "React", "Tailwindcss", "API"],
    },
    {
      organization: "SimWell",
      link: "https://simwell.io",
      role: "Simulation Software Consultant",
      type: "Contract, Part-time",
      location: "Laval, Quebec, Canada",
      startMonth: 1,
      startYear: 2024,
      endMonth: 9,
      endYear: 2024,
      description: ["Distributed microservices and frontend development"],
      skills: [""],
    },
    {
      organization: "Microsoft",
      link: "https://www.microsoft.com/en-us/industry/nuance",
      role: "Software Developer",
      team: "Core Technology R&D, Fabric",
      type: "Internship, Full-time",
      location: "Montreal, Quebec, Canada",
      startMonth: 1,
      startYear: 2023,
      endMonth: 4,
      endYear: 2023,
      description: [
        "Backend API and Microservices development",
        "CI/CD with Docker, kubernetes, and helm on Gitlab and Azure DevOps",
        "Cloud infrastructure management",
      ],
      skills: [
        "k8s",
        "Docker",
        "Linux",
        "Azure DevOps",
        "IAM",
        "GitOps",
        "helm",
      ],
    },
    // {
    //   organization: "Nuance",
    //   link: "https://www.microsoft.com/en-us/industry/nuance",
    //   role: "Software Developer",
    //   team: "Core Technology R&D, Fabric",
    //   type: "Internship, Full-time",
    //   location: "Montreal, Quebec, Canada",
    //   startMonth: 1,
    //   startYear: 2023,
    //   endMonth: 4,
    //   endYear: 2023,
    //   description: ["Now part of Microsoft 🥳"],
    //   skills: [
    //     "k8s",
    //     "Docker",
    //     "Linux",
    //     "Azure DevOps",
    //     "IAM",
    //     "GitOps",
    //     "helm",
    //   ],
    // },
    {
      organization: "Genetec",
      link: "https://genetec.com",
      role: "Software Developer",
      team: "Software Configuration Management & Tools",
      type: "Internship, Full-time",
      location: "Saint-Laurent, Québec, Canada",
      startMonth: 5,
      startYear: 2022,
      endMonth: 8,
      endYear: 2022,
      description: [
        "Managed Azure DevOps, including code repositories, pipelines, APIs, dev tools, automation, and Docker orchestration.",
        "Enhanced personal DevOps abilities, software development practices, and problem-solving skills.",
      ],
      skills: [
        "CI/CD",
        "DevOps",
        "Container Orchestration",
        "IAM",
        "GitOps",
        "C#",
        "Bash",
        "Linux",
        "Shell Scripting",
        "Powershell",
      ],
    },
    {
      organization: "Genetec",
      link: "https://genetec.com",
      role: "IT Specialist",
      team: "IT Operations",
      type: "Internship, Full-time",
      location: "Saint-Laurent, Québec, Canada",
      startMonth: 8,
      startYear: 2021,
      endMonth: 12,
      endYear: 2021,
      description: [
        "On-premise AD and Azure AD management, incident response and investigation, hardware/software asset management and deployment, infrastructure and workflow improvement.",
      ],
      skills: [
        "Active Directory",
        "Jira",
        "Automation",
        "IAM",
        "Technical Support",
        "OS",
        "Powershell",
      ],
    },
    {
      organization: "Concordia University",
      link: "https://www.concordia.ca/ginacody/aits.html",
      role: "IT Technician L2",
      type: "Part-time, Permanent",
      location: "Montréal, Québec, Canada",
      startMonth: 3,
      startYear: 2020,
      endMonth: 12,
      endYear: 2022,
      description: [
        "Troubleshoot user environments on Windows, Linux, macOS, and remote servers.",
        "Address complex hardware, software, and network issues.",
        "Investigate incidents, resolve or escalate.",
        "Manage team's knowledge base, backups, and contribute to university's high-performance computing group.",
      ],
      skills: [
        "Asset Management",
        "Incident Investigation",
        "IAM",
        "Bash",
        "Linux",
        "System Administration",
        "Technical Support",
        "OS",
        "Powershell",
      ],
    },
  ];

  const { data, error } = z.array(ExperienceSchema).safeParse(experiences);

  if (error) {
    throw new Error("Invalid experiences. Unable to parse data: ", error);
  }

  return data;
}
